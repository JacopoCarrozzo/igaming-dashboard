...
export default function KpiCard({ title, value, change, trend, accent }: KpiCardProps) {
  return (
    <div className="relative bg-[#0d1424] border border-[#1e2d45] rounded-xl p-4 overflow-hidden">
      ...
      <p className="text-[26px] font-extrabold text-slate-100 tracking-tight font-mono leading-none">
        {value}
      </p>

      <p className={`text-[11px] font-mono mt-2 ${trend === 'up' ? 'text-emerald-400' : 'text-rose-400'}`}>
        {trend === 'up' ? '↑' : '↓'} {change}
      </p>
